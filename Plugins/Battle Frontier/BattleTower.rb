#===============================================================================
# Battle Tower Scene and Screen
#===============================================================================
class BattleTowerScene
  def initialize
...
class BattleTowerScreen
  ...
  def pbStartChallenge
    ...
    pbMessage(_INTL("Here, you'll face seven trainers in a row."))
    pbMessage(_INTL("Win 21 battles to face Anabel, the Salon Maiden!"))
    ...
    7.times do |i|
      ...
      if decision == 1  # Won
        ...
        # Check if we should face Anabel
        if @streak == 21
          pbMessage(_INTL("Incredible! You've won 21 battles in a row!"))
          pbMessage(_INTL("Anabel, the Salon Maiden, wishes to battle you!"))
          # Create Anabel battle
          brain = BattleFrontierPlugin.createBrainTeam(:TOWER, @streak)
          if brain
            scene = Battle::Scene.new
            battle = Battle.new(scene, $player.party, brain.party, $player, brain)
            decision = battle.pbStartBattle
            if decision == 1
              pbMessage(_INTL("Congratulations! You've defeated Anabel!"))
